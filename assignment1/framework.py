...
class Tanh:
    def forward(self, x):
        ...
        result = np.divide((np.exp(x) - np.exp(-x)),
                           (np.exp(x) + np.exp(-x)))
...
class Linear:
    ...
    def backward(self, error):
        ...
        dW = np.matmul(self.saved_variables['x'].T, error)
        db = np.mean(error, axis=0)

        d_inputs = np.matmul(error, self.saved_variables['W'].T)

        x = self.saved_variables['x'] # To make next assertion work
        ## End
...
class MSE:
    ...
    def backward(self):
        ## Implement
        Y, T, n = self.saved_variables['Y'], self.saved_variables['T'], self.saved_variables['n']

        d_prediction = 1/n * (Y - T)

        y = Y   # To make next code run
        ## End
...
def train_one_step(model, loss, learning_rate, inputs, targets):
    ...
    error = loss.forward(predictions, targets)

    gradient = loss.backward()
    gradient_w_b, gradient_x = model.backward(gradient)

    for k, v in model.var.items():
        model.var[k] = v - learning_rate * gradient_w_b[k]
    ## End
    return error
...
def gradient_check():
    ...
    for key, value in NN.var.items():
        ...
        for index in range(variable.shape[0]):
            ...
            variable[index] = variable[index] + eps
            cost_plus = loss.forward(target=T, prediction=NN.forward(X))

            variable[index] = variable[index] - 2*eps
            cost_minus = loss.forward(target=T, prediction=NN.forward(X))

            analytic_grad = variable_gradient[index]
            numeric_grad = (cost_plus - cost_minus) / (2*eps)
